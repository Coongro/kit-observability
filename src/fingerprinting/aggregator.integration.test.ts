...
describe.skipIf(skipIfNoDb)('recordIssues + preAggregate (integration)', () => {
  ...
  it('UPSERT atómico: misma fingerprint suma occurrence_count en vez de duplicar fila', async () => {
    await recordIssues(sql, [
      { fingerprint: 'fp-x', count: 1, level: 40, source: 's', sampleMessage: 'm1', sampleTopFrame: null, tenantId: null },
    ]);
    await recordIssues(sql, [
      { fingerprint: 'fp-x', count: 5, level: 40, source: 's', sampleMessage: 'm2', sampleTopFrame: null, tenantId: null },
    ]);
    await recordIssues(sql, [
      { fingerprint: 'fp-x', count: 2, level: 40, source: 's', sampleMessage: 'm3', sampleTopFrame: 'frame-A', tenantId: null },
    ]);
    const rows = await sql.unsafe<IssueRow[]>(`SELECT * FROM ${ISSUES_TABLE}`);
    expect(rows).toHaveLength(1);
  ...
  it('preAggregate + recordIssues: 5 entries con 2 fingerprints distintos → 2 filas con counts correctos', async () => {
    const aggregated = preAggregate([
      { fingerprint: 'a', level: 40, source: 's', sampleMessage: 'msg', sampleTopFrame: null, tenantId: null },
      { fingerprint: 'b', level: 40, source: 's', sampleMessage: 'msg', sampleTopFrame: null, tenantId: null },
      { fingerprint: 'a', level: 40, source: 's', sampleMessage: 'msg', sampleTopFrame: null, tenantId: null },
      { fingerprint: 'a', level: 40, source: 's', sampleMessage: 'msg', sampleTopFrame: null, tenantId: null },
      { fingerprint: 'b', level: 40, source: 's', sampleMessage: 'msg', sampleTopFrame: null, tenantId: null },
    ]);
    await recordIssues(sql, aggregated);
    const rows = await sql.unsafe<IssueRow[]>(`SELECT fingerprint, occurrence_count FROM ${ISSUES_TABLE} ORDER BY fingerprint`);
